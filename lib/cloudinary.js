...
export function isAdmin(request) {
  ...
  const cookie = cookieHeader.match(/(?:^|; )gekta_admin=([^;]+)/)?.[1];
...
export async function cloudinaryRequest(path, options = {}) {
  ...
  return fetch(`https://api.cloudinary.com/v1_1/${cloudName}${path}`, { ...options, headers: { Authorization: `Basic ${Buffer.from(`${apiKey}:${apiSecret}`).toString("base64")}`, ...(options.headers || {}) } });
}
